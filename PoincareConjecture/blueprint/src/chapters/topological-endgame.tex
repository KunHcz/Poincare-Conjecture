...
\source{morgan-tian}
\uses{def:simply-connected}
\lean{PoincareConjecture.simplyConnected_threeManifold_orientable,
  PoincareConjecture.simplyConnected_threeManifold_positiveTangentFrames}
\leanok
Every connected simply connected smooth $3$-manifold is orientable.
\end{lemma}

\begin{proof}
Take the actual tangent-bundle coordinate changes, given by derivatives of
the manifold chart transitions.  Their nonzero determinants satisfy the
multiplicative cocycle law.  Their signs therefore glue a discrete two-point
fiber to form the orientation double cover.  Continuity and the covering
property follow from the constructed local trivializations.  Lift the identity
of the simply connected base through this covering, starting at either point
in one fiber.  The resulting continuous section supplies compatible local
orientation signs.  In dimension three, reversing all three axes realizes a
negative sign by an invertible frame map of determinant $-1$.  These local
frame changes make every actual tangent transition determinant positive.
This is the positive-transition criterion for orientability; no orientation
cover or global orientation was assumed as an additional premise.
\end{proof}
...
\dcref{MT:intro:0.1}
\source{morgan-tian}
\lean{PoincareConjecture.freeProduct_factor_subsingleton}
\leanok
...
\source{morgan-tian}
\uses{def:sphere-bundles-over-circle}
\lean{PoincareConjecture.sphere2CircleFundamentalGroupEquiv,
  PoincareConjecture.twistedSphereFundamentalGroupEquiv}
\leanok
Both $E_+$ and $E_-$ have fundamental group isomorphic to $\mathbb Z$.
\end{lemma}

\begin{proof}
For a homeomorphism $\varphi\colon S^2\to S^2$, form the actual orbit
quotient of $S^2\times\mathbb R$ by
$n\cdot(x,t)=(\varphi^n(x),t+n)$.  The action is free.  It is properly
discontinuous because two compact real-coordinate images admit only finitely
many integer translations with nonempty intersection.  The resulting quotient
map is a covering.  Its source is simply connected, using the checked
stereographic-cover proof for $S^2$ and the contractibility of $\mathbb R$.
The covering monodromy is therefore an isomorphism from the fundamental group
of the quotient to $\mathbb Z$ at every basepoint.  Identity and antipodal
monodromy give the two bundle models.  For the identity monodromy, the orbit
relation is the kernel of $(x,t)\mapsto(x,t\bmod\mathbb Z)$; this gives an
actual homeomorphism to $S^2\times(\mathbb R/\mathbb Z)$, followed by the
standard homeomorphism of the additive circle with $S^1$.
\end{proof}
...
\lean{PoincareConjecture.sphere3QuotientDiffeomorph,
  PoincareConjecture.diffeomorph_sphere3_of_finite_quotient}
\leanok
...
\begin{proof}
Write $P=S^3/\Gamma$ as in \ref{def:spherical-space-form}.  The finite free
continuous action gives a quotient covering.  Since $S^3$ is path connected,
monodromy maps $\pi_1(P)$ onto the opposite group of $\Gamma$.
Simple connectivity of $P$ therefore makes $\Gamma$ trivial.  The quotient
projection is consequently bijective.  For the canonical quotient atlas,
local inverse branches followed by the projection agree locally with group
translates; this proves smooth chart transitions, a smooth projection, and
smooth local inverses.  The bijective projection is therefore a global
diffeomorphism.  Notice that simple connectivity of the source sphere is not
needed for this proof.  If $P$ is given by a smooth presentation of the orbit
quotient, compose that presentation with the inverse quotient diffeomorphism.
\end{proof}
...
\textit{Scope of the final target.}  This blueprint theorem is the smooth
version.  The full topological Poincare statement, as required by JSP-000007,
assumes only a compact Hausdorff topological three-manifold and simple
connectivity and concludes a homeomorphism to $S^3$.  Applying this smooth
route to that statement additionally requires a three-dimensional
smoothability theorem.  The exact topological proposition is recorded as
\texttt{PoincareConjecture.TopologicalPoincareStatement}; that definition
does not provide a proof.  Neither version is currently claimed Lean-complete.
