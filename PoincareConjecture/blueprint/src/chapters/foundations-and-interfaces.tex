...
\dcref{MT:ch2:2.22}
\source{morgan-tian}
\lean{PoincareConjecture.dini_le_of_contDiffOn}
\leanok
...
\noindent\textit{Lean interface.}
\texttt{UpperRightDiniLE} states that every strict upper bound on the Dini
derivative eventually bounds the right-hand secant slopes.  It does not
require differentiability of $f$.  The Lean proof obtains a Lipschitz constant
on a compact rectangle containing the graphs of $f$ and $G$, applies mathlib's
one-sided fencing theorem to
$G(t)+\varepsilon e^{(L+1)(t-a)}$, and lets $\varepsilon\downarrow0$.
Only continuity and right derivatives are required of $G$, so the displayed
$C^1$ statement is included.
...
\lean{PoincareConjecture.dini_le_of_finite_jumps}
\leanok
Let
\[
...
continuous on every half-open interval $[\tau_j,\tau_{j+1})$, and satisfies
\[
 f(\tau_j)\le\liminf_{s\uparrow\tau_j}f(s)\qquad(1\le j\le N).
\]
Here the no-upward-jump condition includes the final endpoint $b=\tau_N$.
Equivalently, for every $r<f(\tau_j)$, all sufficiently nearby $s<\tau_j$
satisfy $r<f(s)$.  This formulation does not require a finite left limit.
...
\noindent\textit{Endpoint regression.}
The terminal condition cannot be omitted: for the single interval $[0,1]$,
take $\psi=G=0$, $f(t)=0$ for $t<1$, and $f(1)=1$.  All the half-open
hypotheses hold, but comparison fails at $1$.  This counterexample and the
rejection of its terminal jump by the repaired hypothesis are checked in
\texttt{PoincareConjectureTests.lean}.  The Lean no-upward-jump condition is
\texttt{LowerSemicontinuousWithinAt f (Iio b) b}; a separate test applies the
finite-partition theorem to an actual downward discontinuity.
